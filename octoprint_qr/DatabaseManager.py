...
class DatabaseManager(object):
    ...
    def _createOrUpgradeSchemeIfNecessary(self):
        ...
        try:
            cursor = self.db.execute_sql(
                'select "version" from "qr_pluginmetadatamodel" where key="'
        ...
        if schemeVersionFromDatabase is not None:
            ...
            if currentDatabaseSchemeVersion < CURRENT_DATABASE_SCHEME_VERSION:
                ...
                if self._databaseSettings.useExternal is True:
    ...
    def _handleReusableConnection(
        ...
        try:
            ...
            try:
                if withReusedConnection is False:
                    self._closeDatabase()
            except Exception:
                pass  # do nothing
        pass
    ...
    def loadDatabaseMetaInformations(self, databaseSettings=None):
        ...
        try:
            ...
            if databaseSettings.useExternal is True:
                # External DB
                self._databaseSettings = databaseSettings
                self.connectoToDatabase(sendErrorPopUp=False)
    ...
    def loadQr(self, databaseId, withReusedConnection=False):
        ...
        return self._handleReusableConnection(
            databaseCallMethode, withReusedConnection, "loadQr"
        )
    ...
    def getSpools(self, withReusedConnection=False):
        ...
        return self._handleReusableConnection(
            databaseCallMethode, withReusedConnection, "getSpools"
        )
            ...
            with self._database.atomic() as transaction:  # Opens new transaction.
                try:
                    ...
                    if databaseId is not None:
                        ...
                        currentQrModel = self.loadQr(databaseId, withReusedConnection)
