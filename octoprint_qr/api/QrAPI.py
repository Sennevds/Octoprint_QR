...
class QrManagerAPI(octoprint.plugin.BlueprintPlugin):
    ...
    def _selectSpool(self, databaseId):
        ...
        if databaseId is not None and databaseId != -1:
            ...
            if spoolModel is not None:
                self._logger.info(
                    "Store selected spool '" + spoolModel.code + "' in settings."
                )
                ...
                self._logger.warning(
                    f"Selected Spool with id '{str(databaseId)}"
    ...
    def ReadQR(self):
        ...
        webcamUrl = (
            ...
            if self._settings.get_boolean(["useOctoprintCam"])
            else self._settings.get(["customCamUrl"])
        )
    ...
    def deleteSpool(self, databaseId):
        ...
        if databaseId is not None:
            ...
            databaseId = self._databaseManager.deleteQr(
                databaseId, withReusedConnection=True
            )
            self._databaseManager.closeDatabase()
        else:
    ...
    def select_spool(self):
        ...
        if spoolModel is not None:
            ...
            if (
                self._settings.get_boolean(["sendToSpoolManager"])
                and spoolManager is not None
            ):
                url = f"{request.host_url}plugin/SpoolManager/selectSpool"
                self._logger.info(f"SpoolManager url: {url}")
                payload = json.dumps({"databaseId": spoolModel.spoolManagerId})
                headers = {
