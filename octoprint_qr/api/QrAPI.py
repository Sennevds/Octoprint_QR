...
class QrManagerAPI(octoprint.plugin.BlueprintPlugin):
    ...
    def _updateSpoolModelFromJSONData(self, spoolModel, jsonData):
        ...
        if (self._getValueFromJSONOrNone("databaseId", jsonData) != None):
            spoolModel.code = self._getValueFromJSONOrNone("databaseId", jsonData)
        spoolModel.code = self._getValueFromJSONOrNone("code", jsonData)
        spoolModel.gcodeText = self._getValueFromJSONOrNone("gcodeText", jsonData)
        spoolModel.displayName = self._getValueFromJSONOrNone("displayName", jsonData)
        spoolModel.spoolManagerId = self._toIntFromJSONOrNone("spoolManagerId", jsonData)
        pass
    ...
    def loadSelectedSpool(self, ):
        ...
        if (databaseId != None):
            ...
            self._logger.info(f"connected to database")
    ...
    def _selectSpool(self, databaseId):
        ...
        if (databaseId != None and databaseId != -1):
            ...
            # check if loaded
            if (spoolModel != None):
                self._logger.info("Store selected spool '"+spoolModel.code+"' in settings.")
    ...
    def CreateQR(self):
        ...
        databaseId = self._databaseManager.saveQr(qrModel, withReusedConnection=True)
        self._databaseManager.closeDatabase()
    ...
    def ReadQR(self):
        ...
        qrEngine = QrEngine(self._logger, self._settings.global_get(["webcam", "stream"]))
        code = qrEngine.get_qr()
        self._logger.info(f"code found {code}")
        qr = self._databaseManager.getQrByCode(code, withReusedConnection=False)
        if(qr is None):
            qr = QrModel()
            qr.code = code
    ...
    def saveSpool(self):
        ...
        self._databaseManager.connectoToDatabase()
        if (databaseId != None):
            self._logger.info("Update spool with database code '"+str(databaseId)+"'")
            spoolModel = self._databaseManager.loadQr(databaseId, withReusedConnection=True)
            if (spoolModel == None):
                self._logger.warning("Save spool failed. Something is wrong")
            else:
                self._updateSpoolModelFromJSONData(spoolModel, jsonData)
        ...
        databaseId = self._databaseManager.saveQr(spoolModel, withReusedConnection=True)
        self._databaseManager.closeDatabase()
        return flask.jsonify()
    ...
    def select_spool(self):
        ...
        if (spoolModel != None):
            ...
            spoolManager = self._plugin_manager.get_plugin_info("SpoolManager", require_enabled=True)
